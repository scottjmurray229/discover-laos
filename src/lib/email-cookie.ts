 ...
 *
 * If `secret` is empty/undefined (local dev), falls back to accepting plain `dl_email=1`.
 */
export async function verifyEmailCookie(
  ...
  // Parse the dl_email value out of the cookie header
  const match = cookieHeader.match(/(?:^|;\s*)dl_email=([^;]+)/);
