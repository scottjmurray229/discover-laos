 ...
 *   2. Create project "Discover Laos"
...
function generateMetadata(filename) {
  ...
  if (type === 'dest') {
    return {
      title: `${title} Laos Travel Guide ${year} | Real Prices & Local Tips`,
      ...
      playlist: `${dest?.region || 'laos'}-destinations`,
    };
  }

  if (type === 'pillar') {
    return {
      title: `${title} — Laos ${year}`,
      description: `${title}\n\nFull guide: https://discoverlaos.info/${slug}/`,
      tags: [`Laos ${slug}`, 'Laos travel', `Laos ${year}`],
      categoryId: '19',
  ...
  if (type === 'short') {
    return {
      title: `${title} Laos ✈️ #shorts`,
      description: `${title} — Full guide: https://discoverlaos.info/destinations/${slug}/`,
      tags: [`${title} Laos`, 'Laos travel', 'shorts'],
      categoryId: '19',
      privacyStatus: 'private',
      playlist: 'shorts',
    };
...
function generateDestDescription(slug, title, year) {
  return `Complete travel guide to ${title}, Laos — with real prices, how to get there, where to eat, where to stay, and what most tourists miss.

I've been exploring Laos for years with deep local knowledge. No sponsored content.

📖 FULL WRITTEN GUIDE: https://discoverlaos.info/destinations/${slug}/
📱 TRIP COMPANION (works offline): https://discoverlaos.info/companion/

#Laos #${title.replace(/\s+/g, '')} #LaosTravel #TravelGuide${year}`;
}
...
function generateDestTags(slug, title, dest) {
  ...
  if (dest?.region) tags.push(`${dest.region} Laos`);
...
async function main() {
  console.log('╔══════════════════════════════════════════════════════════╗');
  console.log('║   YOUTUBE UPLOAD — DISCOVER LAOS                   ║');
  console.log('╚══════════════════════════════════════════════════════════╝\n');
